...
function [x, P] = ukf_atc(x, z, P, xNon, xNon0, C, Q, R)
% a) Iniitalize stage
...
% b) Prediction stage
if abs(x(5)) < 1e-5 % Checks for small turn rates
    for i = 1:2*n+1
        X(:,i) = xNon0(X(:,i))*X(:,i); % Calculates predicted sigma points (for small turn rates)
    end
else % For normal turn rates
    for i = 1:2*n+1
        X(:,i) = xNon(X(5,i))*X(:,i); % Calculates predicted sigma points (using normal turn rates)
    end
end
