...
Q_m = diag([2 0.1 2 0.1 2 0.1 2 0.1])*1e0; % Defines Q
R_m = diag([0.1,0.1,0.1,0.1])*1e1; % Defines R
...
for k=2:length(t) %Fill in desired path
...
% %    [x_ukf_r(:,k+1), P_ukf_r(:,:,k+1)] = ukf(x_ukf_r(:,k),z_r(:,k+1), z_kf_m, vshat_r(:,k), P_ukf_r(:,:,k), xNon, C_r, Q_r, R_r);
    ...
    %x_r(:,k+1) = x_r(:,k) + pinv(B_r(x_r(:,k)))*(wr*u_m(:,k+1)+vs_r(:,k))*T;
    x_r(:,k+1) = xNon([x_r(:,k);z_m_real(:,k+1);vs_r(:,k)])+w_r(:,k); % Linear state space equation, %%%%%%%%%%%%%check this
    z_r(:,k+1) = C_r*x_r(:,k)+v_r(:,k); % Linear measurement equation
    
    
%     z_r_ekf=C_r*(xNon([x_ekf_r(:,k);z_kf_m;vs_r(:,k)])+w_r(:,k))+v_r(:,k); %calculate the position based on the filtered motors
%     z_r_onlyekf=C_r*(xNon([x_ekfonly(:,k);z_m_unfiltered;vs_r(:,k)])+w_r(:,k))+v_r(:,k); %calculate the position based on the unfiltered motors
    ...
    [x_ekf_r(:,k+1), P_ekf_r(:,:,k+1)] = ekf(x_ekf_r(:,k), z_r_ekf, z_kf_m, vshat_r(:,k), P_ekf_r(:,:,k), xNon, Alin, C_r, Q_r, R_r); % Calls EKF function to estimate states      
    x_kfonly(:,k+1)=xNon([x_kfonly(:,k);z_kf_m;vs_r(:,k)]);
