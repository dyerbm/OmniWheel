...
P_ukf = P_ekf; % Initializes UKF state error covariance
x_ukf = x_ekf; % Initializes UKF estimates
...
RMSE_ekf = zeros(n,1); % Initializes RMSE vector (EKF)
RMSE_ukf = zeros(n,1); % Initializes RMSE vector (UKF)
error_ekf = (x(:,1) - x_ekf(:,1)).^2; % Initializes squared error (EKF)
error_ukf = (x(:,1) - x_ukf(:,1)).^2; % Initializes squared error (UKF)
...
for k = 1:length(t)-1 % Simulating nonlinear dynamics
    ...
    [x_ukf(:,k+1), P_ukf(:,:,k+1)] = ukf(x_ukf(:,k), z(:,k+1), u(k), P_ukf(:,:,k), xNon, C, Q, R); % Calls UKF function to estimate states  
    error_ekf(:,k+1) = (x(:,k+1) - x_ekf(:,k+1)).^2; % Calculates squared error (EKF)
    error_ukf(:,k+1) = (x(:,k+1) - x_ukf(:,k+1)).^2; % Calculates squared error (UKF)
end
%% Results/plots
for k = 1:n
    RMSE_ekf(k) = (sum(error_ekf(k,:))/length(t))^0.5; % Calculates the RMSE for EKF
    RMSE_ukf(k) = (sum(error_ukf(k,:))/length(t))^0.5; % Calculates the RMSE for UKF
end
fprintf('RMSE (EKF & UKF): %s and %s\n', RMSE_ekf, RMSE_ukf); % Displays RMSE results
% figure; plot(t,u,'LineWidth',2); xlabel('Time (sec)'); ylabel('Input signal'); % Input with time
figure; plot(t,x(1,:),'LineWidth',2); hold all; plot(t,x_ekf(1,:),'--','LineWidth',2); plot(t,x_ukf(1,:),'-.','LineWidth',2); xlabel('Time (sec)'); ylabel('Position (m)'); % Position with time
legend('True','EKF','UKF'); % Adds a legend to figure
