...
for k = 1:tf/T % Applying KF and EKF Estimation Strategies
    ...
    [x_ekf(:,k+1), P_ekf(:,:,k+1)] = imm_ekf(z(:,k+1), 0, UM, CT, 0, H, Q2, R, k ,CTlin, CT0, Hlin); % Calls the EKF Function to calculate the EKF estimates and state error covariance
    [x_ukf(:,k+1), P_ukf(:,:,k+1)] = imm_ukf(z(:,k+1), 0, UM, CT, 0, H, Q2, R, k ,CTlin, CT0, Hlin); % Calls the UKF Function to calculate the EKF estimates and state error covariance
end
...
xlabel('x (m)'); ylabel('y (m)'); title('ATC Estimation Results'); legend('True','KF (UM)','IMM EKF','IMM UKF','Location','SouthEast');

% Creates plot of estimated velocities
figure; plot(t,x(4,:)); hold all; plot(t,x_kf(4,:),'-gx'); plot(t,x_ekf(4,:),'--bx'); plot(t,x_ukf(4,:),'--rx');
xlabel('t'); ylabel('v_y (m/s)'); title('ATC Estimation Results'); legend('True','KF (UM)','IMM EKF','IMM UKF','Location','SouthEast');

figure; plot(t,x(2,:)); hold all; plot(t,x_kf(2,:),'-gx'); plot(t,x_ekf(2,:),'--bx'); plot(t,x_ukf(2,:),'--rx');
xlabel('t'); ylabel('v_x (m/s)'); title('ATC Estimation Results'); legend('True','KF (UM)','IMM EKF','IMM UKF','Location','SouthEast');
